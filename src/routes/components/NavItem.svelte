...
{#if isMobile}
	<div class="w-full">
		<button
			class="min-h-11 px-6 py-2 w-full text-left flex items-center justify-between transition-colors duration-200
             {isActive
		...
		>
			...
			{#if item.dropdownItems}
				<i class="fas fa-chevron-{activeDropdown === item.label ? 'up' : 'down'} text-xs opacity-70"
				></i>
			{/if}
		</button>
	...
	>
		<a
			class="px-3 py-2 rounded-md transition-colors duration-200 flex items-center gap-2
             {isActive
				? 'text-maia-800 dark:text-maia-400'
