...
<a href={link}>
  <div class="flex flex-col p-1">
    ...
    {#each authors as author}
      <p class="text-sm text-gray-600 dark:text-stone-400">{author}</p>
    {/each}
    <div class="mt-3 flex flex-wrap gap-1">
      {#each tags as tag}
        <span class="px-2 py-1 rounded-full text-xs {tag.color} dark:text-white text-black">{tag.title}</span>
      {/each}
    </div>
  </div>
</a>
