<script lang="ts">
	...
	function hasRender(
		content: SectionContent
	): content is SectionContent & { render: () => RenderResult } {
		...
		<SectionContainer id={section.id} title={section.title} icon={section.icon}>
			{#if hasRender(sectionContent)}
				...
				{#if renderResult.components.showML}
					<div
						class="mt-6 p-4 rounded-lg bg-gray-50 dark:bg-maia_black/40 border-l-4 border-purple-400"
					>
						<Aisfml />
					</div>
				{/if}
				{#if renderResult.components.showGov}
					<div
						class="mt-6 p-4 rounded-lg bg-gray-50 dark:bg-maia_black/40 border-l-4 border-fuchsia-400"
					>
						<AisfGov />
					</div>
				{/if}
